	...
	def _get_drumrack_device(self):
		if self._track_controller.selected_track is not None:
			...
			if(track.devices is not None and len(track.devices) > 0):
				...
				if(device is not None and device.can_have_drum_pads and device.has_drum_pads):
				...
				for button, (x, y) in self._matrix.iterbuttons():
					if button:
						...
						if note < 128 and note >= 0:
							if self._drum_group_device is None or (self._drum_group_device is not None and self._drum_group_device.can_have_drum_pads and self._drum_group_device.has_drum_pads and self._drum_group_device.drum_pads[note].chains):
