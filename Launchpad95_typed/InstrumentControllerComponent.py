...
class InstrumentControllerComponent(CompoundComponent):
    def __init__(
        ...
        self._normal_feedback_velocity: int = int(
            self._control_surface._skin["Note.Feedback"]
        )
        self._recordind_feedback_velocity: int = int(
        ...
        self._track_controller = self.register_component(
            TrackControllerComponent(control_surface=control_surface, implicit_arm=True)
        )
    ...
    def handle_scale_name_changed(self) -> None:
        self._scales.set_modus(
            self._scales._modus_names.index(self.song().scale_name), False, True
        )
        self.update()
    ...
    def _set_feedback_velocity(self):
        if self.song().session_record:
            self._control_surface._c_instance.set_feedback_velocity(
                self._recordind_feedback_velocity
            )
        else:
            self._control_surface._c_instance.set_feedback_velocity(
                self._normal_feedback_velocity
            )
    ...
    def _set_swing_amount_value(self, value):
        ...
        self._control_surface.show_message(
            "REPEATER Swing amount: " + str(int(self._swing_amount() * 100)) + "%"
        )
    ...
    def set_scales_toggle_button(self, button):
        ...
        if self._scales_toggle_button is not None:
            self._scales_toggle_button.remove_value_listener(self._scales_toggle)
        self._scales_toggle_button = button
        if self._scales_toggle_button is not None:
            self._scales_toggle_button.add_value_listener(
                self._scales_toggle, identify_sender=True
            )
    ...
    def set_octave_up_button(self, button=None):
        assert isinstance(button, (ButtonElement, type(None)))
        if self._octave_up_button is not None:
            self._octave_up_button.remove_value_listener(self._scroll_octave_up)
        self._octave_up_button = button
        if self._octave_up_button is not None:
            self._octave_up_button.add_value_listener(
                self._scroll_octave_up, identify_sender=True
            )
    ...
    def set_octave_down_button(self, button=None):
        assert isinstance(button, (ButtonElement, type(None)))
        if self._octave_down_button is not None:
            self._octave_down_button.remove_value_listener(self._scroll_octave_down)
        self._octave_down_button = button
        if self._octave_down_button is not None:
            self._octave_down_button.add_value_listener(
                self._scroll_octave_down, identify_sender=True
            )
    ...
    def _scales_toggle(self, value, sender):
        if self.is_enabled():
            if value != 0:
                ...
                if self._scales.is_drumrack and self._drum_group_device is not None:
    ...
    def _matrix_value_quickscale(
        self, value, x, y, is_momentary
    ):  # matrix buttons listener for advanced mode
        if self.is_enabled():
            if self._scales.is_drumrack:
                if (value != 0) or (not is_momentary):
                    if y == 0:
                        if x == 4:
                            ...
                            self._control_surface.show_message(
                                "REPEATER is: "
                                + str("ON" if self._note_repeat.is_enabled() else "OFF")
                            )
                    elif y == 1:
                        if x == 4:
                        ...
                        self._control_surface.show_message(
                            "REPEATER Step: " + str(self._note_repeat.freq_name())
                        )
                    elif y == 3:
                        self._note_repeat.set_freq_index((x - 4) * 2 + y - 2)
                        self._control_surface.show_message(
                    ...
                    if self._quick_scale_root == 0:
                        ...
                        if y == 1 and x < 7 or y == 0 and x in [0, 1, 3, 4, 5]:
                            if y == 1:
                                ...
                                self._control_surface.show_message(
                                    keys[root]
                                    + " "
                                    + str(self._scales._modus_names[selected_modus])
                                )
                            if y == 0 and x < 6:
                            ...
                            if y == 1 and x == 7:  # nav circle of 5th right
                                root = CIRCLE_OF_FIFTHS[
                                    (
                                        self.tuple_idx(CIRCLE_OF_FIFTHS, selected_key)
                                        + 1
                                        + 12
                                    )
                                    % 12
                        ...
                        if y == 0:
                            if x < 7 and self._quick_scales[x] != -1:
                                ...
                                self._control_surface.show_message(
                                    "mode : "
                                    + str(
                                        self._scales._modus_names[self._scales._modus]
                                    )
                                )
                                self.update()
                            if x == 7:
                        ...
                        if y == 1:
                            if x in range(8):
                                ...
                                self._control_surface.show_message(
                                    "REPEATER Step: "
                                    + str(self._note_repeat.freq_name())
                                )
                        self.update()
    ...
    def _update_OSD(self) -> None:
        if self._osd is not None:
            ...
            if (
                self._track_controller is not None
                and self._track_controller.selected_track is not None
            ):
                self._osd.info[0] = (
                    "track : " + self._track_controller.selected_track.name
                )
            else:
                self._osd.info[0] = " "
    ...
    def on_selected_track_changed(self):
        if self._track_controller._implicit_arm:
            self._get_drumrack_device()
            if self._drum_group_device is not None:
    ...
    def _get_drumrack_device(self):
        if self._track_controller.selected_track is not None:
    ...
    def _update_matrix(self):
        if not self.is_enabled() or not self._matrix or self._scales.is_enabled():
            ...
            if (
                self._scales.is_drumrack
                and not self._scales.is_diatonic
                and not self._scales.is_chromatic
            ):
                self._scales.set_drumrack(True)
            else:
                self._scales.set_drumrack(False)
            ...
            if self._scales.is_drumrack:
                ...
                for button, (x, y) in self._matrix.iterbuttons():
                    if button:
                        ...
                        if x < 4:
                            ...
                            note = (
                                16 * (self._scales._octave - 1) + 32 + x + 4 * (7 - y)
                            )
                        ...
                        if note < 128 and note >= 0:
                            if self._drum_group_device is None or (
                                self._drum_group_device is not None
                                and self._drum_group_device.can_have_drum_pads
                                and self._drum_group_device.has_drum_pads
                                and self._drum_group_device.drum_pads[note].chains
                            ):
                            ...
                            if y == 0:
                                if x == 4:
                                    button.set_on_off_values(
                                        "QuickScale.Quant.On", "QuickScale.Quant.Off"
                                    )
                                    ...
                                    button.set_on_off_values(
                                        "QuickScale.NoteRepeater.On",
                                        "QuickScale.NoteRepeater.Off",
                                    )
                                    if self._note_repeat.is_enabled():
                                        button.turn_on()
                                    else:
                                ...
                                if x == 4:
                                    button.set_on_off_values(
                                        "QuickScale.Quant.Straight",
                                        "DefaultButton.Disabled",
                                    )
                                    button.turn_on()
                                elif x == 5:
                                    button.set_on_off_values(
                                        "QuickScale.Quant.Swing",
                                        "DefaultButton.Disabled",
                                    )
                                    button.turn_on()
                                elif x == 6:
                                    button.set_on_off_values(
                                        "QuickScale.Quant.Dotted",
                                        "DefaultButton.Disabled",
                                    )
                                    button.turn_on()
                                elif x == 7:
                                    button.set_on_off_values(
                                        "QuickScale.Quant.Flam",
                                        "DefaultButton.Disabled",
                                    )
                                    button.turn_on()
                            elif y == 2 or y == 3:
                                if x % 2 == 0:
                                    button.set_on_off_values(
                                        "QuickScale.Quant.Selected",
                                        "QuickScale.Quant.Note",
                                    )
                                else:
                                    button.set_on_off_values(
                                        "QuickScale.Quant.Selected",
                                        "QuickScale.Quant.Tripplet",
                                    )
                                if (
                                    (x - 4) * 2 + y - 2
                                ) == self._note_repeat.freq_index():
                                    button.turn_on()
                                else:
                                    button.turn_off()
                        else:
                ...
                if self._scales.is_quick_scale:
                    ...
                    if self._quick_scale_root == KEY_MODE:
                        if selected_modus == 0 or selected_modus == 12:
                            ...
                            relative_scale_button_color = (
                                "QuickScale.Major.RelativeScale"
                            )
                            ...
                            relative_scale_button_color = (
                                "QuickScale.Minor.RelativeScale"
                            )
                        else:
                            ...
                            relative_scale_button_color = (
                                "QuickScale.Other.RelativeScale"
                            )
                        ...
                        for x in range(7):
                            ...
                            if x == 0:
                                ...
                                button.set_on_off_values(
                                    "QuickScale.Quant.Swing", "DefaultButton.Disabled"
                                )
                                button.turn_on()
                            elif x == 4:
                                button.set_on_off_values(
                                    "QuickScale.Quant.Dotted", "DefaultButton.Disabled"
                                )
                                button.turn_on()
                            elif x == 5:
                                button.set_on_off_values(
                                    "QuickScale.Quant.Flam", "DefaultButton.Disabled"
                                )
                                button.turn_on()
                        ...
                        for x in range(8):
                            ...
                            if x % 2 == 0:
                                button.set_on_off_values(
                                    "QuickScale.Quant.Selected", "QuickScale.Quant.Note"
                                )
                            else:
                                button.set_on_off_values(
                                    "QuickScale.Quant.Selected",
                ...
                for button, (i, j) in self._matrix.iterbuttons():
                    if button and (not self._scales.is_quick_scale or j > 1):
                        ...
                        if note_info.index is not None:
                            if note_info.root:
                            ...
                            if note_channel[note_info.index] < 15:
                                note_channel[note_info.index] = (
                                    note_channel[note_info.index] + 1
                                )
                        else:
                            button.set_channel(non_feedback_channel)
    ...
    def tuple_idx(self, target_tuple: tuple[int, ...], obj: int) -> int | bool:
