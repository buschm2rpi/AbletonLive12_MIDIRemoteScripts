...
class DeviceControllerStripProxy:
    ...
    def __init__(
        ...
        self.column: Any = column
        self.request_id: int = 0
        self.server: DeviceControllerStripServer = DeviceControllerStripServer(
            buttons,
            control_surface,
            column,
        ...
        self._server_process: Thread = Thread(target=self.server.run)
    ...
    def _call_non_return_handler(self, name: str, *args: Any, **kwargs: Any) -> None:
        self.request_id += 1
        self._request_queue.put((name, self.request_id, args, kwargs))

    def _call_return_handler(self, name: str, *args: Any, **kwargs: Any) -> Any:
        self.request_id += 1
        current_id: int = self.request_id
        self._request_queue.put((name, current_id, args, kwargs))
        try:
            while True:
                token_response: Any = self._response_queue.get(timeout=10)
                token = token_response[0]
                response = token_response[1]
                if token == current_id:
                    break
                else:
