...
# UNDO button and its listener
	...
	def _undo_button_value(self, value, sender):
		...
		if self.is_enabled() and self._is_pro_mode_on() and self._undo_button is not None:
			now = time.time()
...
# SHIFT Button
	...
	def _shift_button_value(self, value, sender):
		assert (value in range(128))
		if self.is_enabled() and self._is_pro_mode_on() and self._shift_button is not None:
			if ((value != 0) or (not sender.is_momentary())):
...
# CLICK button and its listener
	...
	def _click_value(self, value, sender):
		assert (value in range(128))
		if self.is_enabled() and self._is_pro_mode_on() and self._click_button is not None:
			if ((value != 0) or (not sender.is_momentary())):
...
# QUANTIZE button and its listener
	...
	def _quantize_button_value(self, value, sender):
		assert (value in range(128))
		if self.is_enabled() and self._is_pro_mode_on() and self._quantize_button is not None:
			if ((value != 0) or (not sender.is_momentary())):
...
# DOUBLE button and its listener
	...
	def _double_button_value(self, value, sender):
		assert (value in range(128))
		if self.is_enabled() and self._is_pro_mode_on() and self._double_button is not None:
			if ((value != 0) or (not sender.is_momentary())):
...
# DELETE button and its listener
	...
	def _delete_button_value(self, value, sender):
		assert (value in range(128))
		if self.is_enabled() and self._is_pro_mode_on() and self._delete_button is not None:
			if ((value != 0) or (not sender.is_momentary())):
...
# DUPLICATE button and its listener
	...
	def _duplicate_button_value(self, value, sender):
		assert (value in range(128))
		if self.is_enabled() and self._is_pro_mode_on() and self._duplicate_button is not None:
			if ((value != 0) or (not sender.is_momentary())):
...
# RECORD button and its listener
	...
	def _record_button_value(self, value, sender):
		assert (value in range(128))
		if self.is_enabled() and self._is_pro_mode_on() and self._record_button is not None:
			if self._shift_pressed:
...
# STOP TRACK button and its listener
	...
	def _update_stop_clips_led(self, index):
		...
		elif (self._stop_track_clip_buttons is not None) and (index < len(self._stop_track_clip_buttons)):
	...
	def _update_select_leds(self, index):
		if self._is_pro_mode_on() and (self._stop_track_clip_buttons is not None) and (index < len(self._stop_track_clip_buttons)):
