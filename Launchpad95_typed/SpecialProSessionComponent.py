            ...
            try:
                ...
                if clip is not None:
                    ...
                    try:
                        ...
                        if start_duplicate:
                            view.highlighted_clip_slot.fire(
                                force_legato=True, launch_quantization=_Q.q_no_q
                            )
    ...
    def _do_quantize_clip(self, clip_slot):
        if self._clip_slot and self._clip_slot.has_clip:
            ...
            self._print(
                ...
                + str(
                    REC_QNTZ_NAMES[
                        REC_QNTZ_RATES.index(self._get_record_quantization())
                    ]
                )
            )
            ...
            if button.is_momentary():
                if not self.has_clip():
                    self._clip_slot.fire(
                        record_length=self._get_fixed_length(),
                        launch_quantization=quant,
                    )
                else:
...
class SpecialProSessionComponent(SpecialSessionComponent):
    ...
    def __init__(
        ...
        num_tracks: int,
        num_scenes: int,
        stop_clip_buttons: Optional[tuple[ButtonElement, ...]],
        side_buttons: tuple[ButtonElement, ...],
        control_surface: Any,
        main_selector: Any,
        livesong: Optional[Any] = None,
    ) -> None:
        ...
        SpecialSessionComponent.__init__(
            self,
            num_tracks=num_tracks,
            num_scenes=num_scenes,
            stop_clip_buttons=stop_clip_buttons,
        ...
        self._song.add_clip_trigger_quantization_listener(
            self._on_clip_trigger_quantization_changed_in_live
        )
        ...
        self._record_quantization_on: bool = (
            self._get_song().midi_recording_quantization != Rec_Q.rec_q_no_q
        )

        self._song.add_midi_recording_quantization_listener(
        ...
        self._session_record = SpecialProSessionRecordingComponent(
            target_track_component=TargetTrackComponent(),
    ...
    def disconnect(self) -> None:
        self._song.remove_clip_trigger_quantization_listener(
            self._on_clip_trigger_quantization_changed_in_live
        )
        self._song.remove_midi_recording_quantization_listener(
            self._on_record_quantization_changed_in_live
        )
        self.song().remove_session_record_listener(
            self._on_session_record_changed_in_live
        )
        self._end_undo_step_task.kill()
    ...
    def _set_undo_button(self, button=None):
        ...
        if self._undo_button is not None:
            self._undo_button.add_value_listener(
                self._undo_button_value, identify_sender=True
            )
    ...
    def _undo_button_value(self, value, sender):
        ...
        if (
            self.is_enabled()
            and self._is_pro_mode_on()
            and self._undo_button is not None
        ):
            now = time.time()
    ...
    def _set_shift_button(self, button=None):
        ...
        if self._shift_button != button:
            ...
            if self._shift_button is not None:
                ...
                self._shift_button.add_value_listener(
                    self._shift_button_value, identify_sender=True
                )
    ...
    def _shift_button_value(self, value, sender):
        ...
        if (
            ...
            if (value != 0) or (not sender.is_momentary()):
                self._print(
                    "Select clip - [ Set launch quantization -> now: "
                    + self._get_launch_quant_msg()
                    + " ]"
                )
    ...
    def _set_click_button(self, button=None):
        assert isinstance(button, (ButtonElement, type(None)))
        if self._click_button is not None:
            self._click_button.remove_value_listener(self._click_value)
        self._click_button = button
        if self._click_button is not None:
            self._click_button.add_value_listener(
                self._click_value, identify_sender=True
            )
    ...
    def _click_value(self, value, sender):
        ...
        if (
            self.is_enabled()
            and self._is_pro_mode_on()
            and self._click_button is not None
        ):
            if (value != 0) or (not sender.is_momentary()):
    ...
    def _set_quantize_button(self, button=None):
        ...
        if self._quantize_button != button:
            if self._quantize_button is not None:
            ...
            if self._quantize_button is not None:
                ...
                self._quantize_button.add_value_listener(
                    self._quantize_button_value, identify_sender=True
                )
    ...
    def _quantize_button_value(self, value, sender):
        ...
        if (
            self.is_enabled()
            and self._is_pro_mode_on()
            and self._quantize_button is not None
        ):
            if (value != 0) or (not sender.is_momentary()):
                self._quantize_pressed = True
                if self._shift_pressed:
                    self._print(
                        "Create Scene - [ Record quantization -> now: "
                        + self._get_record_quant_msg()
                        + " ]"
                    )
                else:
                    self._print(
                        "Quantize clip - [ Record quantization -> now: "
                        + self._get_record_quant_msg()
                        + " ]"
                    )
            else:
    ...
    def _get_record_quant_msg(self):
        if self._record_quantization_on:
            return "ON: " + str(
                REC_QNTZ_NAMES[REC_QNTZ_RATES.index(self._record_quantization)]
            )
        else:
            return "OFF"
    ...
    def _get_launch_quant_msg(self):
        if self._launch_quantization_on:
            return "ON: " + str(
                LAUNCH_QNTZ_NAMES[LAUNCH_QNTZ_RATES.index(self._launch_quantization)]
            )
        else:
            return "OFF"
    ...
    def _set_double_button(self, button=None):
        ...
        if self._double_button != button:
            if self._double_button is not None:
                self._double_button.remove_value_listener(self._double_button_value)
            self._double_button = button
            if self._double_button is not None:
                assert isinstance(button, ButtonElement)
                self._double_button.add_value_listener(
                    self._double_button_value, identify_sender=True
                )
    ...
    def _double_button_value(self, value, sender):
        ...
        if (
            self.is_enabled()
            and self._is_pro_mode_on()
            and self._double_button is not None
        ):
            if (value != 0) or (not sender.is_momentary()):
                self._double_pressed = True
                if self._shift_pressed:
                    self._print(
                        "Capture scene - [ Clip fixed length recording -> now: "
                        + self._get_fixed_length_msg()
                        + " ]"
                    )
                else:
                    self._print(
                        "Double midi clip - [ Clip fixed length recording -> now: "
                        + self._get_fixed_length_msg()
                        + " ]"
                    )
            else:
        ...
        if self._delete_button != button:
            ...
            if self._delete_button is not None:
                ...
                self._delete_button.add_value_listener(
                    self._delete_button_value, identify_sender=True
                )
    ...
    def _delete_button_value(self, value, sender):
        ...
        if (
            self.is_enabled()
            and self._is_pro_mode_on()
            and self._delete_button is not None
        ):
            if (value != 0) or (not sender.is_momentary()):
    ...
    def _set_duplicate_button(self, button=None):
        ...
        if self._duplicate_button != button:
            if self._duplicate_button is not None:
                self._duplicate_button.remove_value_listener(
                    self._duplicate_button_value
                )
            self._duplicate_button = button
            if self._duplicate_button is not None:
                assert isinstance(button, ButtonElement)
                self._duplicate_button.add_value_listener(
                    self._duplicate_button_value, identify_sender=True
                )
    ...
    def _duplicate_button_value(self, value, sender):
        ...
        if (
            self.is_enabled()
            and self._is_pro_mode_on()
            and self._duplicate_button is not None
        ):
            if (value != 0) or (not sender.is_momentary()):
    ...
    def _set_record_button(self, button=None):
        ...
        if self._record_button != button:
            ...
            if self._record_button is not None:
                assert isinstance(button, ButtonElement)
                self._record_button.add_value_listener(
                    self._record_button_value, identify_sender=True
                )
    ...
    def _record_button_value(self, value, sender):
        ...
        if (
            self.is_enabled()
            and self._is_pro_mode_on()
            and self._record_button is not None
        ):
            if self._shift_pressed:
                ...
                if (value != 0) or (not sender.is_momentary()):
                    ...
                    if (
                        time.time() - self._last_record_time
                    ) < LONG_PRESS and not self._is_arming:
    ...
    def _update_stop_clips_led(self, index):
        if not self._is_pro_mode_on():
        ...
        elif (self._stop_track_clip_buttons is not None) and (
            index < len(self._stop_track_clip_buttons)
        ):
            ...
            if 0 <= track_index < len(tracks_to_use):
                ...
                if self._record_pressed:
                    ...
                    if not (
                        self._quantize_pressed
                        or self._click_pressed
                        or self._double_pressed
    ...
    def _update_clip_trigger_leds(self, index):
        ...
        if index == 0:
            ...
            if (
                self._get_song().clip_trigger_quantization != _Q.q_8_bars
            ) and self._launch_quantization_on:
                button.send_value("LaunchQuant.PlusMinus.On")
            else:
                button.send_value("LaunchQuant.PlusMinus.Off")
        elif index == 2:
            if (
                self._get_song().clip_trigger_quantization != _Q.q_thirtytwoth
            ) and self._launch_quantization_on:
                button.send_value("LaunchQuant.PlusMinus.On")
            else:
                button.send_value("LaunchQuant.PlusMinus.Off")
    ...
    def _update_rec_qntz_leds(self, index):
        ...
        if index == 0:
            ...
            if (
                self._get_song().midi_recording_quantization != Rec_Q.rec_q_quarter
            ) and self._record_quantization_on:
                button.send_value("RecQuant.PlusMinus.On")
            else:
                button.send_value("RecQuant.PlusMinus.Off")
        elif index == 2:
            if (
                self._get_song().midi_recording_quantization != Rec_Q.rec_q_thirtysecond
            ) and self._record_quantization_on:
                button.send_value("RecQuant.PlusMinus.On")
            else:
                button.send_value("RecQuant.PlusMinus.Off")
        ...
        if self._is_pro_mode_on():
            if self.is_enabled():
                if self._quantize_pressed:
                    ...
                    super(SpecialProSessionComponent, self)._on_stop_track_value(
                        value, button
                    )
                self.update()
        else:
    ...
    def _set_rec_qntz_value(self, value, button):
        if value != 0 or not button.is_momentary():
            ...
            if index == 0:
                ...
                if (
                    self.song().midi_recording_quantization
                    != REC_QNTZ_FIXED_RATES[index - 3]
                ):
                    self.song().midi_recording_quantization = REC_QNTZ_FIXED_RATES[
                        index - 3
                    ]
                else:
                    self._record_quantization_on = not self._record_quantization_on
    ...
    def _set_launch_qntz_value(self, value, button):
        if value != 0 or not button.is_momentary():
            ...
            if index == 0:
                ...
                if (
                    self.song().clip_trigger_quantization
                    != LAUNCH_QNTZ_FIXED_RATES[index - 3]
                ):
                    self.song().clip_trigger_quantization = LAUNCH_QNTZ_FIXED_RATES[
                        index - 3
                    ]
                else:
                    self._launch_quantization_on = not self._launch_quantization_on
    ...
    def _do_arm_track(self, value, button):
        if value != 0 or not button.is_momentary():
            ...
            track_index = (
                list(self._stop_track_clip_buttons).index(button) + self.track_offset()
            )
            if (
                in_range(track_index, 0, len(tracks))
                and tracks[track_index] in self.song().tracks
            ):
                ...
                if track.can_be_armed:
                    if track.arm:
                        ...
                        if (
                            not self._record_mode_on and self._get_song().exclusive_arm
                        ) or (self._record_mode_on and self._armed_track_count == 0):
                            for t in self._get_song().tracks:
                                if t.can_be_armed and t.arm:
                                    t.arm = False
            ...
            if self._fixed_length == 0.25:
                ...
                return (
                    ...
                    + str(
                        int(
                            (
                                self._get_fixed_length()
                                * (self.song().signature_denominator / 4.0)
                            )
    ...
    def set_stop_track_clip_buttons(self, buttons):
        if (not self._is_pro_mode_on()) or buttons is not None:
            super(SpecialSessionComponent, self).set_stop_track_clip_buttons(buttons)

    def _print(self, msg: str) -> None:
        if self._control_surface is not None:
