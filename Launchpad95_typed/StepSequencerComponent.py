...
class StepSequencerComponent(CompoundComponent):
    def __init__(
        ...
        self.QUANTIZATION_COLOR_MAP: list[str] = [
            "StepSequencer.Quantization.One",
            "StepSequencer.Quantization.Two",
            "StepSequencer.Quantization.Three",
            "StepSequencer.Quantization.Four",
        ]
        self.QUANTIZATION_COLOR_MAP_LOW: list[str] = [
            "StepSequencer.QuantizationLow.One",
            "StepSequencer.QuantizationLow.Two",
            "StepSequencer.QuantizationLow.Three",
            "StepSequencer.QuantizationLow.Four",
        ]
    ...
    def _set_mute_shift_function(
        self,
    ):  # Allow to mute notes in the grid or all notes if selecting on Note Selector #FIX bad behavior
    ...
    def _set_loop_selector(self):
        self._loop_selector = self.register_component(
            LoopSelectorComponent(
                self,
                [
                    self._matrix.get_button(4, 4),
                    self._matrix.get_button(5, 4),
                    self._matrix.get_button(6, 4),
                    self._matrix.get_button(7, 4),
                    self._matrix.get_button(4, 5),
                    self._matrix.get_button(5, 5),
                    self._matrix.get_button(6, 5),
                    self._matrix.get_button(7, 5),
                    self._matrix.get_button(4, 6),
                    self._matrix.get_button(5, 6),
                    self._matrix.get_button(6, 6),
                    self._matrix.get_button(7, 6),
                    self._matrix.get_button(4, 7),
                    self._matrix.get_button(5, 7),
                    self._matrix.get_button(6, 7),
    ...
    def _set_note_editor(self):
        self._note_editor = self.register_component(
            NoteEditorComponent(self, self._matrix, self._control_surface)
        )
    ...
    def _set_note_selector(self):
        self._note_selector = self.register_component(
            NoteSelectorComponent(
                self,
                [
                    self._matrix.get_button(0, 7),
                    self._matrix.get_button(1, 7),
                    self._matrix.get_button(2, 7),
                    self._matrix.get_button(3, 7),
                    self._matrix.get_button(0, 6),
                    self._matrix.get_button(1, 6),
                    self._matrix.get_button(2, 6),
                    self._matrix.get_button(3, 6),
                    self._matrix.get_button(0, 5),
                    self._matrix.get_button(1, 5),
                    self._matrix.get_button(2, 5),
                    self._matrix.get_button(3, 5),
                    self._matrix.get_button(0, 4),
                    self._matrix.get_button(1, 4),
                    self._matrix.get_button(2, 4),
                    self._matrix.get_button(3, 4),
                ],
                self._control_surface,
            )
        )
    ...
    def _set_track_controller(self):  # Navigation buttons
        self._track_controller = self.register_component(
            TrackControllerComponent(self._control_surface, implicit_arm=False)
        )
    ...
    def _set_scale_selector(self):
        self._scale_selector = self.register_component(
    ...
    def set_osd(self, osd: Any) -> None:
        self._osd = osd
        if self._scale_selector is not None:
        ...
        if self._osd is not None:
            ...
            if self._selected_track is not None:
                if self._lock_to_track and self._is_locked:
                    self._osd.info[0] = (
                        "track : " + self._selected_track.name + " (locked)"
                    )
                else:
    ...
    def index_of(self, pad_list: list[Any], pad: Any) -> int:
    ...
    def handle_scale_name_changed(self):
        self._scale_selector.set_modus(
            self._scale_selector._modus_names.index(self.song().scale_name), False, True
        )
        self.update()
        ...
        if enabled:
            ...
            if self._drum_group_device:  # Select the note
                self._note_selector.set_selected_note(
                    self.index_of(
                        self._drum_group_device.drum_pads,
            ...
            if self._clip is not None and self._is_locked:
                self._control_surface.show_message(
                    "stepseq : clip '" + str(self._clip.name) + "'"
                )
    ...
    def set_mode(self, mode: int, number_of_lines_per_note: int = 1) -> None:
        if (
            self._mode != mode
            or number_of_lines_per_note != self._number_of_lines_per_note
        ):
            self._number_of_lines_per_note = number_of_lines_per_note
            if self._note_editor is not None:
                self._note_editor.set_multinote(
                    mode == STEPSEQ_MODE_MULTINOTE, number_of_lines_per_note
                )
            if mode == STEPSEQ_MODE_NORMAL:
                if self._mode != mode:
                    # self._loop_selector._block = self._loop_selector._block * self._number_of_lines_per_note
                    if (
                        self._note_editor is not None
                        and self._loop_selector is not None
                    ):
                        self._note_editor.set_page(self._loop_selector._block)
                self.set_left_button(None)
                self.set_right_button(None)
                if self._track_controller is not None and self._top_buttons is not None:
                    self._track_controller.set_prev_track_button(self._top_buttons[2])
                    self._track_controller.set_next_track_button(self._top_buttons[3])
            else:
                if self._mode != mode:
                ...
                if self._top_buttons is not None:
    ...
    def set_page(self, block: int) -> None:
        if self._note_editor is not None:
        ...
        if self._note_selector.is_drumrack:
            ...
            if idx == -1:
                self._control_surface.log_message(
                    "not found : "
                    + str(self._note_selector._offset)
                    + " in "
                    + str(self._note_selector._scale)
                )
                for i in range(8):
                    keys[i] = (
                        self._note_selector._root_note + self._note_selector._offset + i
                    )
            else:
                for i in range(8):
                    keys[i] = (
                        self._note_selector._root_note
                        + self._note_selector._scale[
                            (i + idx) % self._note_selector._scale_length
                        ]
                        + int((i + idx) / self._note_selector._scale_length) * 12
                    )
                    key_is_root_note[i] = (
                        keys[i] + 12
                    ) % 12 == self._note_selector._key
            ...
            for i in range(8):
                ...
                key_is_in_scale[i] = (
                    keys[i] - self._note_selector._key + 12
                ) % 12 in self._note_selector._scale
    ...
    def update(self) -> None:
        if self.is_enabled():
            ...
            self._update_OSD()
            # show clip !
            if not self._is_locked and self._clip is not None:
                if (not self.application().view.is_view_visible("Detail")) or (
                    not self.application().view.is_view_visible("Detail/Clip")
                ):
    ...
    def _update_note_editor(self):
        self._note_editor.set_multinote(
            self._mode == STEPSEQ_MODE_MULTINOTE, self._number_of_lines_per_note
        )
    ...
    def on_selected_track_changed(self):
        if not self._is_locked or self._clip is None:
            self._detect_scale_mode()
            self.on_clip_slot_changed()
            self.update()
    ...
    def on_clip_slot_changed(self, scheduled=False):
        ...
        if not self._is_locked or self._selected_track is None:
            ...
            if self._lock_to_track and self._is_locked:
                ...
                if not scheduled:
                    self._control_surface.schedule_message(
                        5, self.on_clip_slot_changed, (True)
                    )

                # locate with clip pending fire
                for i in range(len(self.song().scenes)):
                    if (
                        self._selected_track.clip_slots[i].has_clip
                        and self._selected_track.clip_slots[i].clip.is_triggered
                    ):
                        idx = i
                ...
                if idx == -1:
                    for i in range(len(self.song().scenes)):
                        if (
                            self._selected_track.clip_slots[i].has_clip
                            and self._selected_track.clip_slots[i].clip.is_playing
                        ):
                            idx = i
                # fallback: use scene selection
                if idx == -1:
                    try:
                        idx = list(self.song().scenes).index(
        ...
        # update clip slot
        if clip_slot != self._clip_slot or self._clip_slot is None:
            if clip_slot is not None and clip_slot.has_clip_has_listener(
        ...
        if (
            self._clip_slot is not None
            and self._clip_slot.has_clip
            and self._clip_slot.clip is not None
            and self._clip_slot.clip.is_midi_clip
        ):
            if self._clip is None or self._clip != self._clip_slot.clip:
                ...
                self._clip_slot.clip.add_playing_status_listener(
                    self._on_playing_status_changed
                )
                self._clip_slot.clip.add_playing_position_listener(
                    self._on_playing_position_changed
                )
    ...
    def _clip_changed(
        self,
    ):  # triggered by _on_clip_slot_changed() or manually on enable.
    ...
    def _on_playing_position_changed(self):  # playing position changed listener
        if self.is_enabled():
            if (
                ...
                self._playhead = None
            self._loop_selector.set_playhead(
                self._playhead, Settings.STEPSEQ__AUTO_SCROLL
            )
    ...
    def _update_drum_group_device(self):
        if self.song().view.selected_track is not None:
            ...
            if track.devices is not None and len(track.devices) > 0:
                ...
                if (
                    device is not None
                    and device.can_have_drum_pads
                    and device.has_drum_pads
                ):  # Is drumrack and it have pads
    ...
    def _detect_scale_mode(self):
        if not self._is_locked:
            self._update_drum_group_device()
            self._scale_selector.set_drumrack(self._drum_group_device is not None)

    def find_drum_group_device(self, track):
        device = find_if(
            lambda d: d.type == Live.Device.DeviceType.instrument, track.devices
        )  # find track's Instrument device
        if device:
            if device.can_have_drum_pads:  # device is a drum rack??
                return device
            elif device.can_have_chains:  # device is a rack??
                return find_if(
                    bool, map(self.find_drum_group_device, device.chains)
                )  # recursive->returns the first drum rack item of the chain
        else:
            return None
    ...
    def _update_scale_selector_button(self):
        if self.is_enabled():
            if self._scale_selector_button is not None:
                if self._clip is not None:
                    self._scale_selector_button.set_on_off_values("StepSequencer.Scale")
                else:
                    self._scale_selector_button.set_on_off_values(
    ...
    def set_scale_selector_button(
        self, button
    ):  # remove old scale button listener and adds new one
        assert isinstance(button, (ButtonElement, type(None)))
        if self._scale_selector_button != button:
            if self._scale_selector_button is not None:
                self._scale_selector_button.remove_value_listener(
                    self._scale_selector_button_value
                )
            self._scale_selector_button = button
            if self._scale_selector_button is not None:
                assert isinstance(button, ButtonElement)
                self._scale_selector_button.add_value_listener(
                    self._scale_selector_button_value
                )
    ...
    def _scale_selector_button_value(self, value):
        ...
        if self.is_enabled():
            if value > 0:
                ...
                if self._scale_selector is not None and self._note_selector is not None:
                    self._scale_selector.set_octave(
                        int(self._note_selector._root_note / 12)
                    )
                    self._scale_selector.set_key(self._note_selector._key)
                    self.set_mode(STEPSEQ_MODE_SCALE_EDIT)
            else:
                if self._scale_selector is not None and self._note_selector is not None:
                    self._note_selector.set_scale(
                        self._scale_selector.notes, self._scale_selector._key
                    )
                    self._note_selector.set_selected_note(
    ...
    def set_mute_shift_button(self, button):
        ...
        if self._mute_shift_button != button:
            if self._mute_shift_button is not None:
                self._mute_shift_button.remove_value_listener(
                    self._mute_shift_button_value
                )
            self._mute_shift_button = button
            if self._mute_shift_button is not None:
                assert isinstance(button, ButtonElement)
                self._mute_shift_button.add_value_listener(
                    self._mute_shift_button_value, identify_sender=True
                )

    def _update_mute_shift_button(self):
        if self.is_enabled() and self._mute_shift_button is not None:
            if self._clip is not None and self._clip.is_midi_clip:
    ...
    def _mute_shift_button_value(self, value, sender):
        assert self._mute_shift_button is not None
        assert value in range(128)
        if self.is_enabled() and self._clip is not None:
            now = time.time()
    ...
    def set_mode_button(
        self, button
    ):  # remove old mode button listener and adds new one
        assert isinstance(button, (ButtonElement, type(None)))
        if self._mode_button != button:
            ...
            if self._mode_button is not None:
                assert isinstance(button, ButtonElement)
                self._mode_button.add_value_listener(
                    self._mode_button_value, identify_sender=True
                )
        ...
        if self.is_enabled() and self._clip is not None:
            if (value != 0) or (not sender.is_momentary()):
                ...
                if (
                    ...
                    and time.time() - self._last_mode_button_press > 0.25
                ):
                    ...
                    self.set_mode(
                        STEPSEQ_MODE_MULTINOTE, self._number_of_lines_per_note
                    )
    ...
    def _update_quantization_button(self):
        if self.is_enabled() and self._quantization_button is not None:
            if self._clip is not None:
                self._quantization_button.set_light(
                    self.QUANTIZATION_COLOR_MAP[self._quantization_index]
                )
            else:
    ...
    def set_quantization_button(self, button):
        ...
        if self._quantization_button != button:
            if self._quantization_button is not None:
                self._quantization_button.remove_value_listener(
                    self._quantization_button_value
                )
            self._quantization_button = button
            if self._quantization_button is not None:
                self._quantization_button.add_value_listener(
                    self._quantization_button_value, identify_sender=True
                )

    # Handle button holded and quantization resolution selection OK
    def _quantization_button_value(self, value, sender):
        assert self._quantization_button is not None
        assert value in range(128)
        if self.is_enabled() and self._clip is not None:
            now = time.time()
            if (value != 0) or (not sender.is_momentary()):
                ...
                if now - self._last_quantize_button_press > 0.5:
                    ...
                    if self._mode == STEPSEQ_MODE_SCALE_EDIT:
                        self._quantization_index = (
                            self._quantization_index - 1 + len(QUANTIZATION_MAP)
                        ) % len(QUANTIZATION_MAP)
                    else:
                        self._quantization_index = (self._quantization_index + 1) % len(
                            QUANTIZATION_MAP
                        )
                    self.set_quantization(QUANTIZATION_MAP[self._quantization_index])
                    self._control_surface.show_message(
                        "QUANTIZATION : " + QUANTIZATION_NAMES[self._quantization_index]
                    )
    ...
    def updateQuantizationButton(self):
        if (
            self.is_enabled()
            and self._quantization_button is not None
            and self._playhead is not None
        ):
            if self._beat == int(self._playhead):
                self._quantization_button.set_light(
                    self.QUANTIZATION_COLOR_MAP_LOW[self._quantization_index]
                )
            else:
    ...
    def set_quantization(self, quantization):
        ...
        if self._loop_selector is not None:
            self._update_loop_selector()
        if self._note_selector is not None:
            self._update_note_selector()
        if self._note_editor is not None:
            self._update_note_editor()
        self._update_OSD()
            ...
            if self._lock_button is not None:
                if self._clip is not None:
                    if self._lock_to_track:
                        self._lock_button.set_on_off_values(
                            "StepSequencer.Lock.ToTrack"
                        )
                    else:
    ...
    def set_lock_button(self, button):
        ...
        if button != self._lock_button:
            ...
            if self._lock_button is not None:
                assert isinstance(button, ButtonElement)
                self._lock_button.add_value_listener(
        ...
        if self.is_enabled() and self._clip is not None:
            ...
            if (value != 0) or (not self._lock_button.is_momentary()):
                ...
                if now - self._last_lock_button_press > self._long_press:
                    ...
                    if not self._is_locked:
                        self._control_surface.show_message(
                            "stepseq : locked to clip '" + str(self._clip.name) + "'"
                        )
        ...
        if button != self._right_button:
            ...
            if self._right_button is not None:
                self._right_button.add_value_listener(
                    self._right_value, identify_sender=True
                )

    def _right_value(self, value, sender):
        assert self._right_button is not None
        assert value in range(128)
        if self.is_enabled() and self._clip is not None:
            if (value != 0) or (not sender.is_momentary()):
        ...
        if button != self._left_button:
            ...
            if self._left_button is not None:
                self._left_button.add_value_listener(
                    self._left_value, identify_sender=True
                )
            ...
            if not clip_slot.has_clip:
                if self._mode == STEPSEQ_MODE_NORMAL:
                    clip_slot.create_clip(
                        QUANTIZATION_MAP[self._quantization_index] * 8 * 4
                    )
                else:
                    clip_slot.create_clip(
                        QUANTIZATION_MAP[self._quantization_index] * 8
                    )
                self._detect_scale_mode()
                clip_slot.fire()
                self.on_clip_slot_changed()
        ...
        if self._clip_slot and self._clip_slot.has_clip:
            try:
                if not self._is_locked or self._lock_to_track:
                    ...
                    newIdx = track.duplicate_clip_slot(
