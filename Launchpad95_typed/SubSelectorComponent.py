...
VOL_VALUE_MAP = tuple(
    sorted([0.0] + [level_to_value(level) for level in Settings.VOLUME_LEVELS])
)
...
class SubSelectorComponent(ModeSelectorComponent):
    ...
    def __init__(
        ...
        self._session: Optional[SessionComponent] = session
        self._mixer: Optional[SpecialMixerComponent] = SpecialMixerComponent(
            matrix.width()
        )
        self._matrix: Optional[ButtonMatrixElement] = matrix
        self._sliders: Optional[list[PreciseButtonSliderElement]] = []
        ...
        self._side_buttons: Optional[Tuple[ButtonElement, ...]] = side_buttons[4:]
    ...
    def on_enabled_changed(self) -> None:
        if self._matrix is None or self._sliders is None or self._mixer is None:
            return
    ...
    def release_controls(self) -> None:
        if self._matrix is None or self._mixer is None or self._session is None:
            return
    ...
    def update(self) -> None:
        ...
        if (
            ...
            or self._sliders is None
            or self._mixer is None
            or self._side_buttons is None
    ...
    def _setup_mixer_overview(self) -> None:
        ...
        self._mixer.set_global_buttons(
            self._side_buttons[3], self._side_buttons[2], self._side_buttons[1]
        )
