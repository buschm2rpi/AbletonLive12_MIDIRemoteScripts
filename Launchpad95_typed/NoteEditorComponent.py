...
class NoteEditorComponent(ControlSurfaceComponent):
    def __init__(
        self,
        stepsequencer: Optional[Any] = None,
        ...
        self._clip_notes: Optional[Any] = None
        ...
        # Velocity color map. this must remain of length 5 (matching velocity_map).
        self.velocity_map: list[int] = [20, 50, 80, 105, 127]
        self.velocity_color_map: list[str] = [
            "StepSequencer.NoteEditor.Velocity0",
            "StepSequencer.NoteEditor.Velocity1",
            "StepSequencer.NoteEditor.Velocity2",
            "StepSequencer.NoteEditor.Velocity3",
    ...
    def update_notes(self) -> None:  # Deprecated ???
    ...
    def _display_selected_page(self):  # OK
        for i in range(0, self._height):
            self._grid_back_buffer[self._page % self.width][i] = (
                "StepSequencer.NoteEditor.PageMarker"
            )
        ...
        for i in range(0, int(self.height / self.number_of_lines_per_note)):
            if self._key_index_is_root_note[i]:
                for j in range(0, self.number_of_lines_per_note):
                    self._grid_back_buffer[0][
                        self.height - i * self.number_of_lines_per_note - j - 1
                    ] = "StepSequencer.NoteEditor.NoteMarker"
                    self._grid_back_buffer[1][
                        self.height - i * self.number_of_lines_per_note - j - 1
                    ] = "StepSequencer.NoteEditor.NoteMarker"
                    self._grid_back_buffer[2][
                        self.height - i * self.number_of_lines_per_note - j - 1
                    ] = "StepSequencer.NoteEditor.NoteMarker"
            ...
            if self._clip is not None and self._note_cache is not None:
                ...
                if self._playhead is not None:
                    ...
                    play_page = int(
                        ...
                        / self.quantization
                        / self.width
                        / self.number_of_lines_per_note
                    )
                    play_row = (
                        int(play_position / self.quantization / self.width)
                        % self.number_of_lines_per_note
                    )
                    play_x_position = (
                        int(play_position / self.quantization) % self.width
                ...
                if self.display_metronome:
                    if self._clip.is_playing and self.song().is_playing:
                        self._grid_back_buffer[play_x_position][play_y_position] = (
                            "StepSequencer.NoteEditor.Metronome"
                        )
                ...
                for note in self._note_cache:
                    note_position = note[
                        1
                    ]  # decimal value of a beat (1=beat, same as playhead)
                    ...
                    note_grid_x_position = (
                        int(note_position / self.quantization) % self.width
                    )
                    note_grid_y_position = (
                        int(note_position / self.quantization / self.width)
                        % self.height
                    )
                    ...
                    if self.is_multinote:
                        ...
                        if note_grid_y_base >= 0:
                            note_grid_y_base = (7 - note_grid_y_base) - (
                                self.number_of_lines_per_note - 1
                            )
                        if note_grid_y_base < 0:
                            note_grid_y_base = -1
                    ...
                    if note_grid_x_position >= 0:
                        ...
                        if (
                            ...
                            and (
                                play_y_position == note_grid_y_position
                            ...
                            self._grid_back_buffer[note_grid_x_position][
                                note_grid_y_position
                            ] = self.playing_note_color
                        elif (
                            note_page == self._page
                        ):  # if note is in current page, then update grid
    ...
    def _matrix_value(self, value, x, y, is_momentary):
        if (
            self.is_enabled() and y < self.height
        ):  # Height value can be 8 (MULTINOTE/SCALE_EDIT) or 4 (STEPSEQ_MODE_NORMAL)
            if (value != 0) or (not is_momentary):  # if NOTE_ON or button is toggle
                self._stepsequencer._was_velocity_shifted = (
                    False  # Some previous state logic INVESTIGATE
                )
    ...
    def _matrix_value_message(
        self, values
    ):  # (value=127/0, x=idx, y=idx, is_momentary=True)
        value = values[0]
        x = values[1]
        y = values[2]
        ...
        if self.is_enabled() and self._clip is None:
            ...
            if value != 0 or not is_momentary:  # if NOTE_ON or button is toggle
                if self._is_velocity_shifted:
                    self._velocity_notes_pressed = (
                        self._velocity_notes_pressed + 1
                    )  # Just changing some note velocity

                # note data

                if self.is_multinote:  # Calculate note pitch and time for notes
                    time = self.quantization * (
                        self._page * self.width * self.number_of_lines_per_note
                        + x
                        + (y % self.number_of_lines_per_note * self.width)
                    )
                    pitch = self._key_indexes[
                        int(
                            8 / self.number_of_lines_per_note
                            - 1
                            - y / self.number_of_lines_per_note
                        )
                    ]
                else:
                    time = self.quantization * (
                ...
                duration = (
                    self.quantization
                )  # setted by quantization button in StepSequencerComponent
                        ...
                        if self._is_velocity_shifted:
                            ...
                            for index in range(len(self.velocity_map)):
                                if note[3] >= self.velocity_map[index]:
                                    new_velocity_index = (index + 1) % len(
                                        self.velocity_map
                                    )
                            note_cache.append(
                                [
                                    ...
                                    note[1],
                                    note[2],
                                    self.velocity_map[new_velocity_index],
                                    note[4],
                                ]
                            )  # (pitch, time, duration, velocity, mute state)
                            ...
                            # mute / un mute note.
                            note_cache.append(
                                [note[0], note[1], note[2], note[3], not note[4]]
                            )  # (pitch, time, duration, velocity, mute state)
                        break
                else:
                    note_cache.append(
                        [pitch, time, duration, velocity, self._is_mute_shifted]
                    )  # (pitch, time, duration, velocity, mute state)
                self._clip.select_all_notes()
    ...
    def _update_velocity_button(self):
        if self.is_enabled() and self._velocity_button is not None:
            if self._clip is not None:
                if self._is_velocity_shifted:
                    self._velocity_button.set_on_off_values(
                        "StepSequencer.NoteEditor.VelocityShifted"
                    )
                    self._velocity_button.turn_on()
                else:
                    self._velocity_button.set_light(
                        "StepSequencer.NoteEditor.Velocity" + str(self._velocity_index)
                    )
            else:
    ...
    def set_velocity_button(self, button):
        ...
        if button != self._velocity_button:
            ...
            if self._velocity_button is not None:
                self._velocity_button.add_value_listener(
                    self._velocity_value, identify_sender=True
                )
        ...
        if self.is_enabled():
            if (value == 0) or (not sender.is_momentary()):
                ...
                if (
                    ...
                    and time.time() - self._velocity_last_press < self.long_button_press
                ):
                    # cycle thru velocities
                    self._velocity_index = (
                        len(self.velocity_map) + self._velocity_index + 1
                    ) % len(self.velocity_map)
            ...
            if (value != 0) or (not sender.is_momentary()):
                ...
                self._stepsequencer._track_controller._implicit_arm = (
                    True  # Arm the track to force play MIDI notes
                )
    ...
    def mute_lane(self, pitch_to_mute: int) -> None:
    ...
    def _display_current_page(self):  # OK
        for i in range(0, self._height):
            if self._page == self._current_page:
                self._grid_back_buffer[self._current_page % self.width][i] = (
                    "StepSequencer.NoteEditor.CurrentPageMarkerPlay"
                )
            else:
                self._grid_back_buffer[self._current_page % self.width][i] = (
                    "StepSequencer.NoteEditor.CurrentPageMarker"
                )
