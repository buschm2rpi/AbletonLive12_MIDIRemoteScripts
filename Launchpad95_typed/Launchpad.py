...
class Launchpad(ControlSurface):
    ...
    def __init__(self, c_instance: Any) -> None:
        ...
        self._selector: Optional[MainSelectorComponent] = (
            None  # needed because update hardware is called
        )
        ...
        with self.component_guard():
            ...
            self._suggested_input_port: Tuple[str, ...] = (
                "Launchpad",
                "Launchpad Mini",
                "Launchpad S",
                "Launchpad MK2",
            ...
            self._suggested_output_port: Tuple[str, ...] = (
                "Launchpad",
                "Launchpad Mini",
                "Launchpad S",
                "Launchpad MK2",
            ...
            self._challenge: int = (
                Live.Application.get_random_int(0, 400000000) & 2139062143
            )
        ...
        self._skin: object
        self._osd: object
        self._note_repeat: NoteRepeatComponent
    ...
    def init(self) -> None:
        ...
        with self.component_guard():
            ...
            if self._mk3_rgb or self._lpx:
                ...
                top_buttons = [
                    ConfigurableButtonElement(
                        is_momentary, MIDI_CC_TYPE, 0, 104 + index, skin=self._skin
                    )
                    for index in range(8)
                ]
                side_buttons = [
    ...
    def disconnect(self) -> None:
        ...
        if self._lpx:
            ...
            self._send_midi(
                STD_MSG_HEADER
                + (LP_X_ID, FIRMWARE_MODE_COMMAND, STANDALONE_MODE, SYSEX_END)
            )
        elif self._mk3_rgb:
    ...
    @staticmethod
    def _combine_active_instances(instance: "Launchpad") -> None:
        support_devices = False
        for inst in Launchpad._active_instances:
            support_devices |= getattr(inst, "_device_component") is not None
        offset = 0
        for inst in Launchpad._active_instances:
            inst._activate_combination_mode(offset, support_devices)
            offset += getattr(getattr(inst, "_selector"), "_session").width()

    def _activate_combination_mode(
        self, track_offset: int, support_devices: bool
    ) -> None:
        ...
        if self._selector is not None and Settings.SESSION__LINK:
            if self._selector._session is not None:
    ...
    def _do_combine(self) -> None:
        if DO_COMBINE and (self not in Launchpad._active_instances):
            ...
            Launchpad._combine_active_instances(self)
    ...
    def _send_challenge(self) -> None:
        ...
        challenge_bytes = tuple(
            [self._challenge >> 8 * index & 127 for index in range(4)]
        )
