...
class LoopSelectorComponent(ControlSurfaceComponent):
    def __init__(
        ...
        self._clip: Any  # clip being played — opaque Ableton object
        ...
        for (
            button
        ) in self._buttons:  # iterate 16 buttons of 4x4 lower right matrix section
    ...
    def set_note_cache(self, note_cache: list[Any]) -> None:
    ...
    def _loop_button_value(self, value: int, sender: ButtonElement) -> None:
        ...
        if self.is_enabled():
            ...
            if (
                value > 0
            ):  # This allow to setup loop range by pressing two buttons at a time [Start,End]
            ...
            # Button released
                ...
                if self._loop_point2 == -1:
                    ...
                    if (
                        ...
                        and (time.time() - self._last_button_time) < 0.25
                    ):  # Double clic set loop subsection (depending on quantization)
                        setloop = True
    ...
    def can_scroll(self, blocks: int) -> bool:
        if self._clip is not None:
            ...
            if (
                ...
                * 8
                * self._quantization
                * self._number_of_lines_per_note
                < self._clip.loop_start
            ):
                return False
            if (
                (blocks + self._block + 1)
                * 8
                * self._quantization
                * self._number_of_lines_per_note
                > self._clip.loop_end
    ...
    def update(self) -> None:
        if self.is_enabled():
            ...
            for (
                ...
                if self._clip is None:  # Disable/turn off all buttons
                    ...
                    in_loop = (
                        i * self._blocksize * self._quantization < self._loop_end
                    ) and (i * self._blocksize * self._quantization >= self._loop_start)
                    # is the playing position is inside the block represented by the button
                    playing = (
                        self._playhead is not None
                        and self._playhead >= i * self._blocksize * self._quantization
                        and self._playhead
                    ...
                    if (
                        self._cache[i] != button._on_value or self._force
                    ):  # Enable/turn on all buttons
    ...
    def _extend_clip_content(
        self,
        loop_start: int | float,
        old_loop_end: int | float,
        new_loop_end: int | float,
    ) -> None:
        if self._no_notes_in_range(old_loop_end, new_loop_end, True):
            ...
            if clip_looping_length > 0:
                ...
                clone_start_point = 0
            self._copy_notes_in_range(
                clone_start_point, clone_start_point + clone_length, old_loop_end
            )

    # Does the note by note copy OK
    def _copy_notes_in_range(
        self, start: int | float, end: int | float, new_start: int | float
    ) -> None:
        ...
        for note in new_notes:
            if note[1] >= start and note[1] < end:
                new_notes.append(
                    [note[0], note[1] + new_start - start, note[2], note[3], note[4]]
                )
        self._clip.select_all_notes()
        self._clip.replace_selected_notes(tuple(new_notes))

    # Checks if a range is empty OK
    def _no_notes_in_range(
        self, start: int | float, end: int | float, or_after: bool
    ) -> bool:
    ...
    def _mute_notes_in_range(self, start: int | float, end: int | float) -> None:
        ...
        for note in list(
            self._note_cache
        ):  # Note -> tuple containing pitch, time, duration, velocity, and mute
