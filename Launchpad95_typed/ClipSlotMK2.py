...
class ClipSlotMK2(ClipSlotComponent):
        ...
        if self._allow_updates:
            if self.is_enabled() and button is not None:
                ...
                if value_to_send is None:
                    button.turn_off()
                elif value_to_send.get("value") in (None, -1):
                    button.turn_off()
    ...
    def _feedback_value(self) -> Optional[Dict[str, Any]]:
        if self._clip_slot is not None:
            ...
            if (
                getattr(slot_or_clip, "controls_other_clips", True)
                and self._stopped_value is not None
            ):
                ret["value"] = self._stopped_value
            if (
                self._track_is_armed(track)
                and self._clip_slot.has_stop_button
                and self._record_button_value is not None
            ):
