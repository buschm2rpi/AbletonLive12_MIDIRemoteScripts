...
class PreciseButtonSliderElement(ButtonSliderElement):
	...
	def _button_value(self, value, sender):
		...
		self._last_sent_value = -1
		if (self._parameter_to_map_to is not None and (not self._disabled) and ((value != 0) or (not sender.is_momentary()))):
			index_of_sender = list(self._buttons).index(sender)
			# handle precision mode
			#if(not self._precision_mode):
			if self._parameter_to_map_to is not None and self._parameter_to_map_to.is_enabled:
