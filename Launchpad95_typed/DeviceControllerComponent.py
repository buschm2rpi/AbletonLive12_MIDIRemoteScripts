...
class DeviceControllerComponent(DeviceComponent):
    ...
    def __init__(
        ...
        name: str = "device_component",
        is_enabled: bool = False,
        matrix: Optional[Any] = None,
        side_buttons: Optional[tuple[ButtonElement, ...]] = None,
        top_buttons: Optional[tuple[ButtonElement, ...]] = None,
    ) -> None:
        self._control_surface: Any
        ...
        # Sliders
        self._sliders: list[Any] | tuple[Any, ...] = []
    ...
    def set_matrix(self, matrix: Optional[ButtonMatrixElement]) -> None:
        self._matrix = matrix
        if self._matrix:
            self._sliders = []
            for column in range(self._matrix.width()):
                slider = DeviceControllerStripProxy(
                    tuple(
                        [
                            self._matrix.get_button(
                                column, (self._matrix.height() - 1 - row)
                            )
    ...
    def set_enabled(self, active: bool) -> bool:
        if active:
            self._force = True
            ...
            if self._selected_track is not None:
                if self._is_locked_to_device:
                    if self._device is not None:
                        self._osd.info[0] = (
                            "track : "
                            + self.get_device_track_name(self._device)
                            + " (locked)"
                        )
                    else:
            ...
            for index in range(len(self._locked_devices)):
                if self._lock_buttons[index] is not None:
                    if self._locked_devices[index] is not None:
                        ...
                        self._lock_buttons[index].set_on_off_values(
                            "Device.Lock.Empty", "Device.Lock.Empty"
                        )  # LED_OFF
        ...
        if self.is_enabled():
            ...
            if value != 0:  # Button down
                ...
                if now - self._lock_button_press[index] > 0.4:  # long press
                    if self._locked_devices[index] is None:  # If lock is available
                        # save locked device
                        dev = -1
                        ...
                        if self._locked_devices[index] is not None:
                            self._control_surface.show_message(
                                ...
                                + self.get_device_track_name(
                                    self._locked_devices[index]
                                )
                                + " - "
                                + str(self._locked_devices[index].name)
                                + "' FROM LOCK: "
                                + str(index + 1)
                            )
    ...
    def update_mode_toggle_button(self):
        if self._mode_toggle_button is not None and self.is_enabled():
            if self._mode_toggle_button is not None:
                if self._device is not None:
                    off_value = (
                        "Device.ModeToggle.Stepless"
                        if self._stepless_mode
                        else "Device.ModeToggle.Normal"
                    )
    ...
    def set_mode_toggle_button(self, button):
        ...
        if self._mode_toggle_button != button:
            if self._mode_toggle_button is not None:
                self._mode_toggle_button.remove_value_listener(self._mode_toggle_value)
            ...
            if self._on_off_button is not None:
                ...
                if parameter is not None:
                    self._on_off_button.set_on_off_values(
                        "Mode.Device.On", "Mode.Device.Off"
                    )
    ...
    def update_track_buttons(self):
        ...
        if self.is_enabled():
            if self._prev_track_button is not None:
                self._prev_track_button.set_on_off_values(
                    "Mode.Track.On", "Mode.Track.Off"
                )

                if (
                    self.selected_track_idx is not None
                    and self.selected_track_idx > 0
                    and not self._is_locked_to_device
                ):
            ...
            if self._next_track_button is not None:
                self._next_track_button.set_on_off_values(
                    "Mode.Track.On", "Mode.Track.Off"
                )
        ...
        if self.is_enabled():
            if (not sender.is_momentary()) or (value != 0):
                if (
                    ...
                    for offset in range(
                        1, len(self.song().tracks) - self.selected_track_idx
                    ):
                        if (
                            self.song()
                            .tracks[self.selected_track_idx + offset]
                            .is_visible
                        ):
    ...
    def update_device_buttons(self):
        ...
        if self.is_enabled():
            if self._prev_device_button is not None:
                self._prev_device_button.set_on_off_values(
                    "Mode.Device.On", "Mode.Device.Off"
                )
            ...
            if self._next_device_button is not None:
                self._next_device_button.set_on_off_values(
                    "Mode.Device.On", "Mode.Device.Off"
                )
                if self.song().appointed_device:
    ...
    def _get_previous_device(self, device):
        ...
        if isinstance(device, Live.Device.Device):
            ...
            elif device.canonical_parent and isinstance(
                device.canonical_parent, Live.Chain.Chain
            ):
    ...
    def _get_previous_device_from_parent(self, device):
        ...
        if isinstance(parent, Live.Chain.Chain):
            ...
            if device in devices and devices.index(device) > 0:
                return self._get_last_device_in_chain(
                    devices[devices.index(device) - 1]
                )
        return None
    ...
    def set_prev_bank_button(self, button):
        ...
        if (
            self._prev_bank_button is not None
            and self._next_bank_button is not None
            or self._prev_bank_button is None
            and self._next_bank_button is None
        ):
